  ...
  PostgreSQL.prototype.getColumnsToDrop = function(model, actualFields) {
    ...
    actualFields.forEach(function(actualField) {
      ...
      if (actualFieldNotPresentInModel(actualField, model)) {
        ...
        console.warn('Skipping DROP COLUMN ' + self.escapeName(actualField.column));
      }
    });
    if (sql.length > 0) {
      sql = [sql.join(', ')];
    ...
    if (fks && fks.length) {
      ...
      fks.forEach(function(fk) {
        ...
        if (needsToDrop) {
          ...
          console.warn('Skipping DROP CONSTRAINT ' + self.escapeName(indexName));
  ...
  PostgreSQL.prototype.addIndexes = function(model, actualIndexes, cb) {
    ...
    aiNames.forEach(function(indexName) {
      ...
      m.properties[propName].index)) {
        ...
        console.warn('Skipping DROP INDEX ' + schema + '.' + self.escapeName(indexName));
      } else {
